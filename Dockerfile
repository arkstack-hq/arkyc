...
# Assemble a standalone artifact (flat node_modules, independent of the workspace)
# into /app.
...
# NOTE: deliberately NOT `--prod`. The `ark` CLI (@h3ravel/musket) statically
# imports `tsdown` (a devDependency), so a prod-only artifact cannot run `ark` at
# all — it dies with ERR_MODULE_NOT_FOUND. The container needs `ark` for
# `migrate` (on startup) and `queue:work` (the worker), so the dev deps stay.
...
RUN pnpm --filter=@arkyc/api deploy --legacy /app \
