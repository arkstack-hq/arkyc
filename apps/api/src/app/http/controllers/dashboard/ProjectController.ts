...
export default class ProjectController extends BaseController {
    ...
    async index ({ req }: HttpContext) {
        ...
        return new ProjectCollection(projects).additional({ status: 'success', message: 'OK', code: 200 })
    }
    ...
    async create ({ req }: HttpContext) {
        ...
        if (await Project.where({ tenantId: req.tenant!.id, slug }).first()) {
            throw new AppException('A project with this slug already exists in this tenant', 409)
        }
        ...
        return new ProjectResource(project)
            .additional({ status: 'success', message: 'Project created', code: 201 })
            .response()
            .setStatusCode(201)
    }
    ...
    async show ({ req }: HttpContext) {
        ...
        return new ProjectResource(project).additional({ status: 'success', message: 'OK', code: 200 })
    }
    ...
    async update ({ req }: HttpContext) {
        ...
        return new ProjectResource(project).additional({ status: 'success', message: 'Project updated', code: 200 })
    }
}
