    ...
    public async up (schema: SchemaBuilder): Promise<void> {
        schema.createTable('verification_sessions', (table) => {
            ...
            table.uuid('tenantId').map('tenant_id')
                .foreign().references('tenants', 'id').onDelete('cascade').as('tenant').inverseAlias('sessions')
            table.uuid('projectId').map('project_id')
                .foreign().references('projects', 'id').onDelete('cascade').as('project').inverseAlias('sessions')
            ...
            table.uuid('reviewedBy').nullable().map('reviewed_by')
                .foreign().references('users', 'id').onDelete('setNull').as('reviewer').inverseAlias('reviewedSessions')
