    ...
    public async up (schema: SchemaBuilder): Promise<void> {
        schema.createTable('document_captures', (table) => {
            ...
            table.uuid('tenantId').map('tenant_id')
                .foreign().references('tenants', 'id').onDelete('cascade').as('tenant').inverseAlias('documentCaptures')
            table.uuid('projectId').map('project_id')
                .foreign().references('projects', 'id').onDelete('cascade').as('project').inverseAlias('documentCaptures')
            table.uuid('sessionId').map('session_id')
                .foreign().references('verification_sessions', 'id').onDelete('cascade').as('session').inverseAlias('documentCaptures')
