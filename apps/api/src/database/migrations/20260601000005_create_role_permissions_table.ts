    ...
    public async up (schema: SchemaBuilder): Promise<void> {
        schema.createTable('role_permissions', (table) => {
            ...
            table.uuid('roleId').map('role_id')
                .foreign().references('roles', 'id').onDelete('cascade').as('role').inverseAlias('rolePermissions')
            table.uuid('permissionId').map('permission_id')
                .foreign().references('permissions', 'id').onDelete('cascade').as('permission').inverseAlias('rolePermissions')
