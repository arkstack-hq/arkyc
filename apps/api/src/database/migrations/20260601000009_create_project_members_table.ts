    ...
    public async up (schema: SchemaBuilder): Promise<void> {
        schema.createTable('project_members', (table) => {
            ...
            table.uuid('tenantId').map('tenant_id')
                .foreign().references('tenants', 'id').onDelete('cascade').as('tenant').inverseAlias('projectMembers')
            table.uuid('projectId').map('project_id')
                .foreign().references('projects', 'id').onDelete('cascade').as('project').inverseAlias('members')
            table.uuid('userId').map('user_id')
                .foreign().references('users', 'id').onDelete('cascade').as('user').inverseAlias('projectMemberships')
            table.uuid('roleId').map('role_id')
                .foreign().references('roles', 'id').onDelete('restrict').as('role').inverseAlias('projectMembers')
            table.string('status')
