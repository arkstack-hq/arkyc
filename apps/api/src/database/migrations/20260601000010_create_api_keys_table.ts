    ...
    public async up (schema: SchemaBuilder): Promise<void> {
        schema.createTable('api_keys', (table) => {
            ...
            table.uuid('tenantId').map('tenant_id')
                .foreign().references('tenants', 'id').onDelete('cascade').as('tenant').inverseAlias('apiKeys')
            table.uuid('projectId').map('project_id')
                .foreign().references('projects', 'id').onDelete('cascade').as('project').inverseAlias('apiKeys')
            table.string('name')
