...
export const SIGNATURE_HEADER = 'X-Arkyc-Signature';

/** Header carrying the unix-seconds timestamp the signature covers. */
export const TIMESTAMP_HEADER = 'X-Arkyc-Timestamp';
...
export function verifyWebhookSignature(input: VerifyWebhookInput): boolean {
  ...
  const expected = signWebhook(input.payload, input.secret, input.timestamp);
